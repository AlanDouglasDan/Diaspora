...
import { useUserProfileViewLogic } from "./useUserProfileViewLogic";

const UserProfileView: FC<UserProfileViewScreenProps> = (props) => {
  const {
    avatar,
    displayName,
    prefs,
  ...
  return (
    <View style={{ flex: 1 }}>
      <SuccessNotification
        visible={showSuccess}
        title={`Love letter sent to ${displayName}`}
        message="You've sent a love letter. Your message will be displayed in their priority aisle."
      ...
      >
        ...
        <View style={styles.loveLetterContainer}>
          ...
          <TouchableOpacity
            style={[
              styles.shareButton,
              (!loveLetterText.trim() || isSendingLetter) && { opacity: 0.6 },
            ]}
            activeOpacity={0.7}
            onPress={handleSendLoveLetter}
            disabled={!loveLetterText.trim() || isSendingLetter}
          >
            {isSendingLetter ? (
